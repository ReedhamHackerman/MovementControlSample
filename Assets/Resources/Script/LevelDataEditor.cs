...
public class LevelDataEditor : Editor
{
    ...
    private void OnEnable()
    {
        ...
        list.onAddDropdownCallback = (Rect buttonRect, ReorderableList l) => {
            ...
            guids = AssetDatabase.FindAssets("", new[] { "Assets/Resources/Prefabs/Bosses" });
            foreach (var guid in guids)
            {
                var path = AssetDatabase.GUIDToAssetPath(guid);
                menu.AddItem(new GUIContent("Bosses/" + Path.GetFileNameWithoutExtension(path)), false, clickHandler, new WaveCreationParams() { Type = MobWave.WaveType.Boses, Path = path });
            }
            menu.ShowAsContext();
        };
    }
    ...
    private void clickHandler(object target)
    {
        ...
        element.FindPropertyRelative("Count").intValue = data.Type == MobWave.WaveType.Boses ? 1 : 20;
